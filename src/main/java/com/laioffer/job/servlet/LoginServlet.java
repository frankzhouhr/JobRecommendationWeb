...
public class LoginServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // user info saved at cookie's session
        if (connection.verifyLogin(body.userId, body.password)) {
            HttpSession session = request.getSession(); // Create session only when login
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ObjectMapper mapper = new ObjectMapper();
        HttpSession session = request.getSession(false); // get Current session and don't create a new one if it does not exist
        LoginResponseBody loginResponseBody;
        if (session != null) { // if session exists => Check if logged in
            MySQLConnection connection = new MySQLConnection();
